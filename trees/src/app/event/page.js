...
export default function EventPage() {
  const [step, setStep] = useState(1); //لأتبع الخطوات
  const [selectedRegion, setSelectedRegion] = useState(null); //لأتبع المنطقة
  const [events, setEvents] = useState([]); // لأتبع تخزين الحدث
  const [selectedEvent, setSelectedEvent] = useState(null); //لأتبع الحدث المحدد
  const router = useRouter(); //لأتنقل بين الصفحات
  //-----------------------
  //todoo -------------- لأحضر كل ال event --------------
  const fetchEvents = async (region) => {
    try {
      const response = await axios.get(`/api/events/${region}`); //لأحضر الحدث حسب المنطقة المحددة
      setEvents(response.data); //تخزين بال state
    } catch (error) {
      console.error("Error fetching events:", error);
    }
  };
  //todoo -------------- لحتى اختار المنطقة --------------
  const handleRegionSelect = (region) => {
    setSelectedRegion(region); //تحديث المنطقة المحددة
    fetchEvents(region); //احضار ال event لهاي المنطقة
    setStep(2); //انتقال للخطوة التانية
  };
  //todoo -------------- لحتى اسجل بال event --------------
  ...
  //todoo -------------- لحتى اتأكد من التسجيل بال event --------------
          ...
          <div className="lg:absolute lg:left-[26%] lg:top-1/2 lg:transform lg:-translate-x-1/2 lg:-translate-y-1/2 mt-4 w-full lg:w-[350px] h-auto lg:h-[400px] bg-white shadow-lg z-10 p-8 rounded order-1 lg:order-3">
            ...
            {step === 2 && (
              <>
                ...
                <p>
                  ...
                  us to transform our environment into a flourishing green
                  haven.
                </p>
              </>
            )}
            ...
            {step === 4 && (
              <>
                ...
                <p>
                  We are excited to have you with us as we embark on this
                  journey to create a more sustainable and beautiful world
                  together.
                </p>
              </>
            )}
